...
_ACTIVE_PATRONS_QUERY = """
    SELECT
        ...
        deletion_date_gmt, record_last_updated_gmt, creation_date_gmt
    FROM (
        SELECT id, record_last_updated_gmt, deletion_date_gmt, creation_date_gmt
    ...
    LEFT JOIN sierra_view.patron_view ON x.id = patron_view.id
...
def build_redshift_address_query(address_hashes):
    return _REDSHIFT_ADDRESS_QUERY.format(
        redshift_table=os.environ["REDSHIFT_TABLE"], address_hashes=address_hashes
    )


def build_redshift_initial_codes_query(patron_ids):
    return _REDSHIFT_INITIAL_CODES_QUERY.format(
        redshift_table=os.environ["REDSHIFT_TABLE"], patron_ids=patron_ids
    )
