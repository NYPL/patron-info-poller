...
def reformat_malformed_address(address_row):
    """
    Parses the original address and uses the parsed version to set the city, region,
    etc. that should be sent to the geocoders
    """
    ...
    try:
        ...
        address_row["address"] = (
            address_row["house_number"]
            + " "
            + address_row["street_name"]
            + " "
            + parsed_address.get("line2", "")
        ).strip()
        ...
        for df_field, label in [
            ...
            address_row[df_field] = (
                _combine_repeated_labels(e.parsed_string, label)
                or address_row[df_field]
            )
        ...
        address = (
            address_row["house_number"] + " " + address_row["street_name"] + " " + line2
        ).strip()
        if len(address) > 0:
            address_row["address"] = address

    # Strip the city and region of anything that's not a letter, space, or -.
    address_row["city"] = re.sub(
        "[^A-Za-zÀ-ÖØ-öø-ÿ-\\s]", "", unidecode(address_row["city"])
    ).strip()
    address_row["region"] = re.sub(
        "[^A-Za-zÀ-ÖØ-öø-ÿ-\\s]", "", unidecode(address_row["region"])
    ).strip()
    # Strip the street_name and address of anything that's not a letter, space, digit,
    # or common punctuation.
    address_row["street_name"] = re.sub(
        "[^A-Za-zÀ-ÖØ-öø-ÿ0-9-\\s#&.,;:+@/]", "", unidecode(address_row["street_name"])
    ).strip()
    address_row["address"] = re.sub(
        "[^A-Za-zÀ-ÖØ-öø-ÿ0-9-\\s#&.,;:+@/]", "", unidecode(address_row["address"])
    ).strip()
    # Strip the postal_code of anything that's not a digit or -.
    address_row["postal_code"] = re.sub(
        "[^\\d-]", "", unidecode(address_row["postal_code"])
    ).strip()
...
def _combine_repeated_labels(parsed_string, label):
    """
    When the parsed address contains multiple portions with the same label, concatenate
    them unless they are identical.

    Returns None if the output is empty
    """
    output_list = []
    for addr_portion in parsed_string:
        if addr_portion[1] == label and addr_portion[0] not in output_list:
...
def _combine_multilabel_field(parsed_string, labels):
    """
    For repeated labels that all map to the same field (e.g. the multiple portions of
    the street_name), combine them all separately using _combine_repeated_labels() and
    ...
    output_list = []
    for label in labels:
        parsed_label = _combine_repeated_labels(parsed_string, label)
