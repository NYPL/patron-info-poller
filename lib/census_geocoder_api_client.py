...
class CensusGeocoderApiClient:
    ...
    def get_geoids(self, address_df):
        """
        Sends the addresses in address_df to the geocoder a single time, which may
        require multiple requests if the geocoder is overloaded.

        Returns a series containing the geoids (or NaN) indexed to match address_df.
        """
        self.logger.info(
            "Sending ({}) addresses to census geocoder API".format(len(address_df))
        )
        raw_response = self._send_request(address_df)
        response_df = pd.read_csv(
            BytesIO(raw_response),
            header=None,
            dtype=str,
            index_col=0,
            engine="python",
        ...
        geoids = (
            response_df["state_id"] + response_df["county_id"] + response_df["tract_id"]
        ).rename("geoid")
        return geoids

    def _send_request(self, address_df):
        """
        Send a request to the API. Recursively calls itself with a smaller batch size if
        the initial request fails.

        Returns a csv string where each line contains information about a single
        geocoded address.
        """
        try:
            with BytesIO() as address_stream:
                ...
                self.logger.debug(
                    "Sending {}-address batch to geocoder API".format(len(address_df))
                )
                address_stream.seek(0)
                response = self.session.post(
                    os.environ["GEOCODER_API_BASE_URL"],
                    files={
                        "addressFile": NamedTextIOWrapper(
            ...
            if new_df_size >= 1000:
                self.logger.info(
                    "Initial geocoding request failed -- sending two new requests with "
                    "{} addresses each".format(new_df_size)
                )
...
class NamedTextIOWrapper(TextIOWrapper):
    """
    Wrapper class around TextIOWrapper that makes the 'name' attribute settable. The
    geocoder API apparently requires a named file to function.
    """
