...
class PipelineController:
    """
    Class for orchestrating different types of pipeline runs. There are three modes:
    1) checking for newly created patrons, 2) checking for recently updated patrons (who
    may or may not have changed their address), and 3) checking for recently deleted
    patrons
    """
    ...
    def run_pipeline(self, mode):
        ...
        while not finished:
            ...
            self.logger.info(
                (
                    "Begin processing {mode} patrons batch {batch} with state {state}"
                ).format(mode=mode, batch=batch_number, state=self.poller_state)
            )
    ...
    def _run_active_patrons_single_iteration(self, mode):
        ...
        unprocessed_sierra_df["patron_id_plaintext"] = (
            unprocessed_sierra_df["patron_id_plaintext"]
            .astype("Int64")
            .astype("string")
        )

        # Check that the number of records with the same timestamp does not exceed the
        # batch size, which would cause the poller to stall.
        if len(unprocessed_sierra_df) == int(
            os.environ["ACTIVE_PATRON_BATCH_SIZE"]
        ) and (
            (
                mode == PipelineMode.NEW_PATRONS
                and min(unprocessed_sierra_df["creation_timestamp"])
                == max(unprocessed_sierra_df["creation_timestamp"])
            )
            or (
                mode == PipelineMode.UPDATED_PATRONS
                and min(unprocessed_sierra_df["last_updated_timestamp"])
        ...
        unseen_records_mask = ~unprocessed_sierra_df["patron_id_plaintext"].isin(
            self.processed_ids
        )
        processed_df = unprocessed_sierra_df[unseen_records_mask].reset_index(drop=True)

        # If there are no unprocessed patron ids left, return. Otherwise, update the
        # total set of processed ids.
        if len(processed_df) == 0:
            return None
        self.processed_ids.update(processed_df["patron_id_plaintext"])

        # Reduce the dataframe to only one row per patron_id, keeping the row with the
        # lowest display_order and patron_record_address_type_id
        distinct_records_mask = ~processed_df.duplicated(
            "patron_id_plaintext", keep="first"
        )
        processed_df = processed_df[distinct_records_mask].reset_index(drop=True)

        # Obfuscate the patron addresses using bcrypt
        self.logger.info(
            "Concatenating and obfuscating ({}) addresses".format(len(processed_df))
        )
        processed_df[["address", "city", "region", "postal_code"]] = processed_df[
            ["address", "city", "region", "postal_code"]
        ].astype("string")
        processed_df["address_hash_plaintext"] = (
            processed_df["patron_id_plaintext"]
            + "_"
            + processed_df["address"].fillna("")
            + "_"
            + processed_df["city"].fillna("")
            + "_"
            + processed_df["region"].fillna("")
            + "_"
            + processed_df["postal_code"].fillna("")
        ).astype("string")
        with ThreadPoolExecutor() as executor:
            processed_df["address_hash"] = list(
                executor.map(obfuscate, processed_df["address_hash_plaintext"])
            )

        # For every (patron id + address) hash found in Redshift, use the geoid and
        # obfuscated patron id found there
        if mode == PipelineMode.UPDATED_PATRONS:
            ...
            processed_df[["initial_patron_home_library_code", "initial_ptype_code"]] = (
                processed_df[["patron_home_library_code", "ptype_code"]]
            )
            processed_df["is_in_redshift"] = False
            processed_df["is_in_redshift"] = processed_df["is_in_redshift"].astype(
                "bool"
            )

        # For every row not already in Redshift, obfuscate the patron id and geocode it
        ...
        if len(unknown_patrons_df) > 0:
            ...
            if mode == PipelineMode.UPDATED_PATRONS:
                initial_codes_df = self._find_initial_codes(
                    processed_df.loc[~processed_df["is_in_redshift"], "patron_id"]
                )
                processed_df = processed_df.merge(
                    initial_codes_df, how="left", on="patron_id"
                )
                processed_df.loc[
                    ~processed_df["is_in_redshift"], "initial_patron_home_library_code"
                ] = processed_df["iphlc"]
                processed_df.loc[
                    ~processed_df["is_in_redshift"], "initial_ptype_code"
                ] = processed_df["ipc"]

        # Modify the data to match what's expected by the PatronInfo Avro schema, encode
        # it, and send it to Kinesis
        processed_df["postal_code"] = processed_df["postal_code"].str.slice(stop=5)
        processed_df["creation_date_et"] = processed_df["creation_timestamp"].dt.date
    ...
    def _run_deleted_patrons_single_iteration(self):
        ...
        # Get data from Sierra
        query = build_deleted_patrons_query(
            self.poller_state["deletion_date"], self.now
        )
        self.sierra_client.connect()
        sierra_raw_data = self.sierra_client.execute_query(query)
        self.sierra_client.close_connection()
        unprocessed_sierra_df = pd.DataFrame(
            data=sierra_raw_data, columns=["patron_id_plaintext", "deletion_date_et"]
        )
        unprocessed_sierra_df["patron_id_plaintext"] = (
            unprocessed_sierra_df["patron_id_plaintext"]
            .astype("Int64")
        ...
        # Check that the number of records with the same date does not exceed the batch
        # size, which would cause the poller to stall.
        if len(unprocessed_sierra_df) == int(
            os.environ["DELETED_PATRON_BATCH_SIZE"]
        ) and (
            ...
            raise PipelineControllerError("Too many records found with the same date")
        ...
        self.logger.info("Obfuscating ({}) patron ids".format(len(processed_df)))
        with ThreadPoolExecutor() as executor:
            processed_df["patron_id"] = list(
                executor.map(obfuscate, processed_df["patron_id_plaintext"])
            )

        # Take the existing data in Redshift for each deleted patron and merge it with
        # the deletion date
    ...
    def _find_known_addresses(self, all_patrons_df):
        """
        Checks if any of the (patron id + address) hashes already appear in Redshift. If
        they do, take the geoid and obfuscated patron id from Redshift and join it with
        the original Sierra dataframe.
        """
        ...
        new_all_patrons_df = all_patrons_df.merge(
            redshift_df, how="left", on="address_hash"
        )
        new_all_patrons_df["is_in_redshift"] = (
            new_all_patrons_df["is_in_redshift"].astype("boolean").fillna(False)
        )
        return new_all_patrons_df

    def _find_deleted_patrons(self, deleted_patrons_df):
        """
        Finds the Redshift data for recently deleted patrons and joins it with the
        deletion date from Sierra.
        """
        patron_ids_str = "','".join(
        ...
        self.redshift_client.close_connection()
        redshift_df = pd.DataFrame(data=redshift_raw_data, columns=_REDSHIFT_COLUMNS)

        full_patrons_df = deleted_patrons_df.merge(
    ...
    def _process_unknown_patrons(self, unknown_patrons_df):
        """
        Takes a dataframe of patrons whose addresses have not already been geocoded,
        obfuscates their patron ids, sends them to the census geocoder API and then, if
        that's unsuccessful, to the NYC geocoder.
        """
        # Obfuscate the patron ids using bcrypt
        address_df = unknown_patrons_df.copy()
        self.logger.info("Obfuscating ({}) patron ids".format(len(address_df)))
        with ThreadPoolExecutor() as executor:
            address_df["patron_id"] = list(
                executor.map(obfuscate, address_df["patron_id_plaintext"])
            )

        # Get geoids from census geocoder API
        address_df[["address", "city", "region", "postal_code"]] = (
            address_df[["address", "city", "region", "postal_code"]]
        ...
        # For addresses that weren't geocoded, reformat them and try again. Sending two
        # requests is also recommended by the API because it sometimes erroneously fails
        # to match an address when in batch mode. For more info, see:
        ...
        self.logger.info(
            "Successfully geocoded {success}/{total} non-empty addresses".format(
                success=len(geoids[geoids.notnull()]), total=len(geoids)
            )
        )
        address_df["geoid"] = geoids
        return address_df[["patron_id", "geoid"]]
    ...
    def _find_initial_codes(self, unknown_initial_codes_series):
        """
        Finds the initial patron home library code and ptype code for existing patrons
        whose addresses could not be found in Redshift
        """
        patron_ids_str = "','".join(
    ...
    def _get_poller_state(self, batch_number):
        ...
        if not self.ignore_cache:
            ...
            return {
                "creation_dt": os.environ.get(
                    "STARTING_CREATION_DT", "2020-01-01 00:00:00-05"
                ),
                "update_dt": os.environ.get(
                    "STARTING_UPDATE_DT", "2020-01-01 00:00:00-05"
                ),
                "deletion_date": os.environ.get("STARTING_DELETION_DATE", "2020-01-01"),
            }
        else:
